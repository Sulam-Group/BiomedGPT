...
class CaptionsEvaluation:
    ...
    def _load_data(self):
        gold_csv = pd.read_csv(
            self.gold_dir,
            sep="\t",
            header=None,
            names=["unique_ids", "image_ids", "captions", "labels", "image_codes"],
            dtype=object,
        )
    ...
    def _preprocess_captions(self, images_caption_dict):
        ...
        for image_id in images_caption_dict:
            processed_captions_dict[image_id] = [
                _bioclean(images_caption_dict[image_id])
            ]
    ...
    def compute_ms_coco(self):
        ...
        if len(self.gold_data) == len(self.result_data):
            ...
            logging.info(
                "Gold data len={0} and results data len={1} have not equal size".format(
                    len(self.gold_data), len(self.result_data)
                )
            )
