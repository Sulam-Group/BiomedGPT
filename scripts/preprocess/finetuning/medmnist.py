...
parser.add_argument(
    "--mode", type=int, default=0, help="0: convert numpy to png, or 1: preprocess"
)
        ...
        for name in files:
            if name.endswith(".npz"):
                ...
                train_data_name = os.path.join(
                    save_path_of_the_directory, data_cat + "_train.tsv"
                )
                val_data_name = os.path.join(
                    save_path_of_the_directory, data_cat + "_val.tsv"
                )
                test_data_name = os.path.join(
                    save_path_of_the_directory, data_cat + "_test.tsv"
                )

                path_of_the_directory_sub = os.path.join(
                    path_of_the_directory, data_cat
                )
                with open(train_data_name, "w") as out_train, open(
                    val_data_name, "w"
                ) as out_val, open(test_data_name, "w") as out_test:
                    ...
                    val_count = 0
                    test_count = 0
                    for path_sub, dir_sub, files_sub in os.walk(
                        path_of_the_directory_sub
                    ):
                        for name_sub in files_sub:
                            ...
                            if "train" in path_sub:
                                label_index = str(
                                    train_labels[int(name_sub[:-4]) - 1][0]
                                )  # np.int8
                                ...
                                label_index = str(
                                    test_labels[int(name_sub[:-4]) - 1][0]
                                )  # np.int8
                                label_name = INFO_DICT[data_cat]["label"][label_index]
                                out_test.write(
                                    base64_str
