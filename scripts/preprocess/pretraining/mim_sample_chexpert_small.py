...
parser.add_argument(
    ...
    type=int,
    default=0,
    help="0: resize to (256*256), or 1: capture middle part (128*128) string, or 2: base64 string, or 3: combine image-code via vq-gan",
)
...
if args.mode == 0:
    ...
    save_path_of_the_directory = (
        "../../../datasets/pretraining/CheXpert-v1.0-small/train_resize"
    )
...
if args.mode == 1:
    ...
    count = 0
    with open(
        "../../../datasets/pretraining/chexpert_mid_image_string.tsv", "w"
    ) as out_file:
        for path, dirc, files in os.walk(path_of_the_directory):
            for name in files:
                if name.endswith(ext):
...
if args.mode == 2:
    ...
    count = 0
    with open(
        "../../../datasets/pretraining/chexpert_image_string.tsv", "w"
    ) as out_file:
        for path, dirc, files in os.walk(path_of_the_directory):
            for name in files:
                if name.endswith(ext):
...
if args.mode == 3:
    ...
    """
    with open(
        "../../../datasets/pretraining/chexpert_image_string.tsv", "r"
    ) as f1, open(".../../../datasets/pretraining/chexpert_image_code.tsv", "r") as f2:
        with open("../../../datasets/pretraining/chexpert.tsv", "w") as out:
            for x, y in zip(
                csv.reader(f1, delimiter="\t"), csv.reader(f2, delimiter="\t")
            ):
